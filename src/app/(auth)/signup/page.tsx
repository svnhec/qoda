  ...
  return (
    ...
    >
      ...
      >
        ...
        <h1 className="text-2xl font-bold tracking-tight mb-1">Initialize Account</h1>
        <p className="text-sm text-muted-foreground">Create your Qoda identity</p>
      </motion.div>
          ...
          <div className="relative">
            <Input
              ...
              className={cn(
                ...
                "focus:border-primary focus:ring-primary/20",
                "placeholder:text-muted-foreground/50",
                password && (isStrong ? "border-primary/50" : "border-warning/50"),
              )}
            />
            <button
              type="button"
                  ...
                  <div key={req.label} className="flex items-center gap-2 text-xs">
                    <motion.div
                      className={cn(
                        ...
                        req.check ? "bg-primary/20 text-primary" : "bg-white/5 text-muted-foreground",
                      )}
                      animate={{ scale: req.check ? [1, 1.2, 1] : 1 }}
                      transition={{ duration: 0.2 }}
                    >
                      {req.check && <Check className="w-2.5 h-2.5" />}
                    </motion.div>
                    <span className={req.check ? "text-primary" : "text-muted-foreground"}>{req.label}</span>
                  </div>
                ))}
              </motion.div>
            )}
